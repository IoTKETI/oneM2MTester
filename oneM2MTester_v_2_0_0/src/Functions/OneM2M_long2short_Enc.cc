...
namespace OneM2M__DualFaceMapping {
	...
	const std::string ANYTYPE_ANY_1 = "any_1"; //TODO to be removed
	...
	CHARSTRING f__primitiveContent__Dec(const CHARSTRING& source_str, const CHARSTRING& serial_type){
		...
		if("json" == serial_type){
			...
			jsonArrayRoot.append(jsonRootClone);

			StyledWriter writer;
			std::string json_str = writer.write(jsonRootClone);

			StyledWriter writer2;
			std::string json_array_str = writer2.write(jsonArrayRoot);

			CHARSTRING temp_array_cs(json_array_str.c_str());
			encoded_message = temp_array_cs;
			...
			if(jsonRoot.isObject()){
				...
				for (Value::iterator iter = jsonRoot.begin(); iter != jsonRoot.end(); ++iter) {
					...
					if(elemObj.isArray()){
						...
						////////////////////////////////
						///// TO Be Removed
						///// anytype "any_1" delete
						//// Eclipse titan cannot
						///  decode anytype which
						// results any_1 is encode  
						// as normal type. 
						// Here we remove the any_1
						// from the Json document	
						///////////////////////////////
						if(elemObj.isMember(ANYTYPE_ANY_1.c_str())){ 

							subelemObj = elemObj.get(ANYTYPE_ANY_1, ""); 

							 if(subelemObj.isArray()){
								for(unsigned int index = 0; index < subelemObj.size(); index++){

									grandelemObj = subelemObj[index];
									jsonObjClone[parent_tag.c_str()] = grandelemObj;
								}
							  }
	...
	Json::Value JSONDeepParser(Json::Value jsonSrc, Json::Value jsonObjClone, Json::Value jsonTarget){
		...
		for (Value::iterator iter = jsonSrc.begin(); iter != jsonSrc.end(); iter++) {
			...
			if(elemObj.isObject()){ 
				...
				TTCN_Logger::log(TTCN_DEBUG, "elemObj.isString()");				
				...
				if(	"op" 	== name_short   || "operation" 			== name_short || 
					...
					"csy"	== name_short	|| "consistencyStrategy" 	== name_short  ){ //TODO: add all enumerated type here
