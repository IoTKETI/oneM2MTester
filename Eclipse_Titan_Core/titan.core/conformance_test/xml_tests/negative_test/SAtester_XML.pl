...
The compile block instructs the SAtester_XML to:
...
A module block instructs the SAtester_XML to produce a source module
...
A result block instructs the SAtester_XML how to check result as soon the actions in
   ...
   if (scalar @sa_tcList2Execute == 0) {
      foreach my $filename (@sa_scriptFiles) {
         my $Buffer = sa_readFile($filename);
		  ...
		  #replacing tmp keywords with actual files' content
		  my $replacementFile = 'XSD.txt';
		  my $replacementFile2 = 'UsefulTtcn3Types.txt';

		  my $xsdKey = "xsd_tmp_keyword";
		  my $usefulKey = "usefultypes_tmp_keyword";

		  my $replacement = sa_readFile($replacementFile);
		  my $replacement2 = sa_readFile($replacementFile2);

		  $Buffer =~ s/$xsdKey/$replacement/g;
		  $Buffer =~ s/$usefulKey/$replacement2/g;

		 #sa_log("buffer: $Buffer \n"); #megcsinalja  replacet csak valahol nem replacelt hasznalja !
